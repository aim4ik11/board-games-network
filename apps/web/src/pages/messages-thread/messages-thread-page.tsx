...
import styles from './messages-thread-page.module.scss';
...
export function MessagesThreadPage() {
  ...
  return (
    <section className={`page ${styles.page}`}>
      ...
      {messagesQuery.data && (
        <div className={`button-row ${styles.threadHead}`}>
          ...
          <div className={styles.messages}>
            {messagesQuery.data.data.map((m, index, all) => {
              ...
              return (
                <div
                  key={m.id}
                  className={`${styles.bubbleWrap} ${mine ? styles.mine : styles.theirs}`}
                >
                  {!mine && (
                    <div
                      className={`${styles.avatar} ${!isLastInSenderRun ? styles.avatarGhost : ''}`.trim()}
                      aria-hidden
                    >
