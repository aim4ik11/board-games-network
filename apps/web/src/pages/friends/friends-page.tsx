...
import pageStyles from './friends-page.module.scss';
...
export function FriendsPage() {
  ...
  return (
    <section className={`page ${pageStyles.page}`}>
      <h1>Friends</h1>
      <nav className={seg.root} aria-label="Friends section">
        {TABS.map((t) => (
          <button
            key={t.value}
            type="button"
            className={t.value === tab ? seg.tabActive : seg.tab}
            onClick={() => {
              if (t.value === 'discover') {
                void navigate({
